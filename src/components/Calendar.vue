  ...
  <div class="p-4">
    <div class="flex items-center justify-between mb-3">
      ...
      <div class="flex items-center gap-2">
          <button @click="prevMonth" aria-label="Bulan sebelumnya" class="inline-flex items-center justify-center h-8 w-8 rounded bg-white border text-gray-700 hover:shadow hover:bg-gray-50 transition-colors duration-150">
          ...
          <button @click="goToToday" class="px-2 py-1 rounded bg-teal-600 text-white text-sm hover:brightness-95">Hari ini</button>
          <button @click="nextMonth" aria-label="Bulan berikutnya" class="inline-flex items-center justify-center h-8 w-8 rounded bg-white border text-gray-700 hover:shadow hover:bg-gray-50 transition-colors duration-150">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="none" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 4l6 6-6 6" />
            </svg>
          </button>
          <button @click="refresh" title="Segarkan" class="ml-2 inline-flex items-center gap-2 px-2 py-1 rounded bg-white border text-sm hover:shadow group transition-colors duration-150 hover:bg-gray-50 active:scale-95">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transform transition-transform duration-200 ease-in-out group-hover:rotate-90" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-9-9m0 0v4m0-4h4"/>
            </svg>
          </button>
        </div>
    </div>
    ...
    <div class="mb-2 flex items-center gap-2">
      <div v-if="isLoading" class="text-xs text-gray-500 flex items-center gap-2">
      ...
      <div v-else-if="fetchError" class="text-xs text-rose-600">Gagal memuat jadwal</div>
    </div>
    ...
    <div class="grid grid-cols-7 gap-1 mt-2">
      ...
      >
          <div class="flex-none h-10 flex items-center justify-center">
            <div :class="[ isHoliday(cell.date) ? 'text-rose-700' : (isToday(cell.date) ? 'text-teal-700' : 'text-gray-700') ]" class="text-center">
              <span class="inline-flex items-center justify-center w-8 h-8 leading-none text-base align-middle" :class="badgeClassForDate(cell.date)">
                {{ cell.date.getDate() }}
              </span>
            </div>
          </div>

          <div v-if="eventsFor(cell.date).length" class="w-full mt-2 cell-events">
            <ul class="space-y-1 text-left text-[11px]">
              <li v-for="(ev, i) in eventsFor(cell.date).slice(0,2)" :key="i" :class="['truncate px-1 rounded text-white', ev.isHoliday ? 'bg-rose-600/90' : 'bg-teal-600/90']">{{ ev.title }}</li>
              <li v-if="eventsFor(cell.date).length>2" class="text-[11px] text-gray-500">+{{ eventsFor(cell.date).length - 2 }} lainnya</li>
            </ul>
          </div>
      </div>
    </div>
    ...
    <teleport to="body">
      <div
        ...
        role="tooltip"
      >
        <div class="tooltip-inner bg-white border rounded-lg shadow-lg p-2 text-xs text-gray-800 relative">
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative z-10 w-full max-w-md bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="flex items-center justify-between px-4 py-3 border-b">
          <div>
            <div class="text-sm font-medium text-gray-900">Kegiatan - {{ selectedDateLabel }}</div>
        ...
        <div class="p-4">
          <ul v-if="selectedEvents.length" class="space-y-3">
            <li v-for="(ev, i) in selectedEvents" :key="i" class="p-3 rounded-lg border">
                  ...
                  <div class="mt-1 text-xs text-gray-500">
                    ...
                    <span v-if="ev.timeZone" class="ml-2 font-medium">{{ ev.timeZone }}</span>
                  </div>
                  <div v-if="ev.desc" class="mt-2 text-sm text-gray-600" v-html="ev.desc"></div>
            </li>
          </ul>
          <div v-else class="text-sm text-gray-500">Tidak ada kegiatan pada tanggal ini.</div>
        </div>
        <div class="px-4 py-3 border-t text-right">
          <button @click="closeModal" class="px-3 py-1 rounded bg-teal-600 text-white">Tutup</button>
        </div>
      </div>
    </div>
  </div>
...
function cacheKeyFor(y, m) { return `calendar_events_${y}_${m}`; }
...
function saveCache(y, m, items) {
  try { localStorage.setItem(cacheKeyFor(y,m), JSON.stringify({ ts: Date.now(), items })); } catch(e){}
}

// Holiday cache (daily)
const holidayDatesRef = ref(new Set());
const HOLIDAY_CACHE_TTL_MS = 1000 * 60 * 60 * 24; // 24 hours
function holidayCacheKeyFor(y, m) { return `holiday_dates_${y}_${m}`; }
...
function saveHolidayCache(y, m, items) {
  try { localStorage.setItem(holidayCacheKeyFor(y,m), JSON.stringify({ ts: Date.now(), items })); } catch(e){}
}
...
async function fetchHolidaysForView(force = false) {
  try {
    ...
    const parsedDates = [];
    items.forEach(it => {
      const ds = it.date || it.tanggal || it.day || it.day_string || it.dayDate || it.day_date || it.dayName || null;
      if (ds) {
        try {
          const d = new Date(ds);
    ...
    try { saveHolidayCache(y, m, parsedDates); } catch(e){}
  } catch (err) {
...
function normalizeServerEvent(e) {
  ...
  if (e.date) dateStr = e.date;
    ...
    if (typeof e.start === 'string') {
      try { dateStr = formatKey(new Date(e.start)); } catch (err) { dateStr = null; }
    } else if (e.start.date) {
      dateStr = String(e.start.date);
    } else if (e.start.dateTime) {
      try { dateStr = formatKey(new Date(e.start.dateTime)); } catch (err) { dateStr = null; }
    }
  ...
  if (!time && e.start) {
    try {
      const dt = (typeof e.start === 'string') ? new Date(e.start) : (e.start && e.start.dateTime ? new Date(e.start.dateTime) : null);
      if (dt && !isNaN(dt)) time = dt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    } catch (err) { time = null; }
  }

  // Detect holiday by server flag, organizer/creator email, or keywords in title/summary
  const titleLower = String(title).toLowerCase();
  const holidayKeywords = /\blibur\b|hari libur|\bcuti\b|nasional|national|idul|lebaran|nyepi|tahun baru|kemerdekaan|hari raya|liburan/i;
  const creatorEmail = (e.creator && e.creator.email) ? String(e.creator.email).toLowerCase() : '';
  const organizerEmail = (e.organizer && e.organizer.email) ? String(e.organizer.email).toLowerCase() : '';
  const isHoliday = (e.holiday === true) || (e.type === 'holiday') || holidayKeywords.test(titleLower) || /holiday|libur/.test(creatorEmail) || /holiday|libur/.test(organizerEmail);
  ...
  try {
    if (startObj.dateTime) {
      ...
      if (!isNaN(sd)) startTimeStr = sd.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      // derive tz label from offset if not provided
      if (startObj.timeZone) tzLabel = startObj.timeZone;
      else if (!tzLabel && sd && !isNaN(sd)) tzLabel = mapOffsetToIndoLabel(-sd.getTimezoneOffset() / 60);
    ...
    if (endObj.dateTime) {
      ...
      if (!isNaN(ed)) endTimeStr = ed.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      if (!tzLabel) {
        if (endObj.timeZone) tzLabel = endObj.timeZone;
        else if (ed && !isNaN(ed)) tzLabel = mapOffsetToIndoLabel(-ed.getTimezoneOffset() / 60);
      }
    } else if (endObj.date) {
      // all-day
      endTimeStr = null;
  ...
  return { id: e.id, date: dateStr, title, time: time || startTimeStr, startTime: startTimeStr, endTime: endTimeStr, timeZone: shortTz, desc: e.description || e.desc || '', isHoliday };
}
...
function mapOffsetToIndoLabel(offset) {
  ...
  if (typeof offset === 'number' && !isNaN(offset)) return `UTC${offset >= 0 ? '+' + offset : offset}`;
  return null;
}
...
function mapToShortIndoTz(tz) {
  ...
  if (s.includes('jakarta') || s.includes('wib') || s.includes('asia/jakarta')) return 'WIB';
  if (s.includes('makassar') || s.includes('wita') || s.includes('asia/makassar') || s.includes('asia/denpasar')) return 'WITA';
  if (s.includes('jayapura') || s.includes('wit') || s.includes('asia/jayapura')) return 'WIT';
...
const monthYear = computed(() => {
  return new Intl.DateTimeFormat('id-ID', { month: 'long', year: 'numeric' }).format(viewDate.value);
});
...
const cells = computed(() => {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    out.push({ key: formatKey(d), date: d, isCurrentMonth: d.getMonth() === viewDate.value.getMonth() });
  }
  return out;
});
...
async function fetchEventsForView(force = false) {
  ...
  try {
    ...
    if (!force) {
      ...
      if (cached) {
        eventsRef.value = cached.map(normalizeServerEvent).filter(e => !!e.date);
    ...
    const url = `/cmb/auto-calendar/fetch?period=${y}-${m}`;
    const headers = {
    ...
    if (data && typeof data === 'object' && data.error && String(data.error).toLowerCase().includes('google token not found')) {
      try {
        const proceed = (typeof Swal !== 'undefined')
          ? await Swal.fire({ title: 'Akun Google belum terhubung', text: 'Anda akan diarahkan untuk menghubungkan akun Google.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Buka koneksi', cancelButtonText: 'Batal' }).then(r=>r.isConfirmed)
    ...
    const list = Array.isArray(data)
      ? data
      : (Array.isArray(data.events) ? data.events : (Array.isArray(data.items) ? data.items : []));
    // Save raw list to cache for this month
    try { saveCache(y, m, list); } catch(e){}
...
function buildTooltipContentFor(date) {
  const human = new Intl.DateTimeFormat('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(date);
  const evs = eventsFor(date);
  let html = `<div class="font-medium mb-1">${escapeHtml(human)}</div>`;
  if (evs && evs.length) {
    html += '<ul class="space-y-1">';
    evs.slice(0, 3).forEach(ev => {
      const timeText = ev.startTime ? `${escapeHtml(ev.startTime)}${ev.endTime ? (' - ' + escapeHtml(ev.endTime)) : ''}` : '';
      const tzText = ev.timeZone ? `${escapeHtml(ev.timeZone)}` : '';
      const badge = ev.isHoliday ? '<span class="text-rose-600">●</span> ' : '';
      html += `<li class="pb-1">
        <div class="font-medium">${badge} ${escapeHtml(ev.title)}</div>
        <div class="text-gray-500 text-[11px] mt-0.5">${timeText ? escapeHtml(timeText) : ''}${timeText && tzText ? ' ' : ''}${tzText ? '(' + escapeHtml(tzText) + ')' : ''}</div>
      </li>`;
    });
    if (evs.length > 3) html += `<li class="text-gray-500">+${evs.length - 3} lainnya</li>`;
    html += '</ul>';
  } else {
...
function onCellPointerDown(evt, cell) {
  try {
    const pType = evt && evt.pointerType ? evt.pointerType : (evt.type === 'touchstart' ? 'touch' : 'mouse');
...
const selectedDateHuman = computed(() => {
  return selectedDate.value
    ? new Intl.DateTimeFormat('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(selectedDate.value)
    : '';
});
...
/* Tooltip styles */
.calendar-tooltip { transform-origin: top left; }
...
@media (max-width: 640px) {
  .calendar-tooltip { max-width: 85vw; }
}
</style>
