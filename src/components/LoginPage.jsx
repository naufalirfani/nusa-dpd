...
function LoginPage() {
  ...
  function sendCode() {
    if (!identifier) {
      if (typeof window.Swal !== 'undefined') window.Swal.fire({ icon: 'warning', title: t('enter_nip') });
    ...
    if (isEmail) {
      ...
      if (!ok) {
        if (typeof window.Swal !== 'undefined') window.Swal.fire({ icon: 'warning', title: t('invalid_nip') });
    ...
    setTimeout(() => {
      handleGenerateCode();
      setSending(false);
      if (typeof window.Swal !== 'undefined') window.Swal.fire({ icon: 'success', title: t('code_generated') });
  ...
  async function onSubmit(e) {
    ...
    if (!code) {
      if (typeof window.Swal !== 'undefined') window.Swal.fire({ icon: 'warning', title: t('enter_code') });
    ...
    if (code.trim() === verificationCode) {
        ...
        try {
          ...
          if (typeof window.Swal !== 'undefined') {
            await window.Swal.fire({
              ...
              text: t('token_generate_failed') || 'Token generation failed — continuing in demo mode.',
            });
          }
        } finally {
          setLoading(false);
      ...
      if (typeof window.Swal !== 'undefined') window.Swal.fire({ icon: 'error', title: t('verify_fail') });
              ...
              <form onSubmit={onSubmit} className="mt-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <canvas
                      ...
                      width="220"
                      height="48"
                      className="h-12 w-[220px] rounded-xl border border-gray-200 bg-white/70 shadow-sm cursor-pointer"
                    ...
                    <button
                      ...
                      className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 to-sky-500 px-4 text-white shadow-sm transition hover:from-teal-600 hover:to-sky-600 disabled:opacity-50 disabled:shadow-none"
                    >
                      {sending ? (
                        ...
                        <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                          <path d="M21 12.79A9 9 0 1111.21 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  ...
                  <div className="mt-3">
                    <div className="relative">
                      <input
                        ...
                        className="w-full rounded-xl border border-gray-200 bg-gray-50/70 px-4 py-3 pr-12 text-gray-900 placeholder:text-gray-400 shadow-sm outline-none focus:border-teal-400 focus:bg-white focus:ring-2 focus:ring-teal-100"
                        placeholder="XXXXXX"
                      ...
                      >
                        {!showCode ? (
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 5 12 5c4.638 0 8.573 2.507 9.963 6.676.07.207.07.431 0 .639C20.577 16.49 16.64 19 12 19c-4.638 0-8.573-2.507-9.964-6.678z" />
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                          </svg>
                        ) : (
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.324 16.169 7.26 18.676 11.9 18.676c1.67 0 3.26-.307 4.703-.862M6.228 6.228A10.45 10.45 0 0111.9 5.324c4.64 0 8.576 2.507 9.966 6.676a10.522 10.522 0 01-1.278 2.3" />
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 00-3-3m0 0a3 3 0 013 3m-3-3L3 21m9-12l9 9" />
                          </svg>
                        )}
                      </button>
                    </div>
                ...
                <div>
                  <button
                    ...
                    disabled={loading}
                    aria-busy={loading}
                    className="group inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 to-sky-500 px-5 py-3 font-medium text-white shadow-sm transition hover:from-teal-600 hover:to-sky-600 disabled:opacity-50"
                  >
                    {loading && (
                      <svg className="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
