...
// We always call the same-origin proxy path `/cmb-sso/...`. In development
// Vite will forward `/cmb-sso` to the backend, and in production nginx will
...
async function createJwt(payload = {}, expiresInSeconds = 3600) {
  ...
  const url = `/cmb-sso/generate/${encodeURIComponent(nip)}?${params.toString()}`;
