      ...
      >
        ...
        <form @submit.prevent="onSubmit" class="space-y-4">
          ...
          <div>
            <button
              type="submit"
              class="w-full px-4 py-2 rounded-md bg-teal-500 hover:bg-teal-600 text-white font-medium"
            >
              {{ t("login") }}
            </button>
          </div>
...
// JWT helpers (client-side HMAC-SHA256 signing using SubtleCrypto)
const JWT_SECRET = import.meta.env.VITE_JWT_SECRET || "dev_secret_replace_me";
...
async function signHMAC(message, secret) {
  // Web Crypto (SubtleCrypto) works only in secure contexts (https or localhost).
  // When accessing the app via a LAN IP over http, `crypto.subtle` may be unavailable.
  // In that case return a deterministic fallback signature (dev-only).
  if (typeof window === 'undefined' || !window.crypto || !crypto.subtle) {
    try { console.warn('[auth] SubtleCrypto unavailable — using dev fallback signature'); } catch (e) {}
    // simple fallback: HMAC not available; return a fixed-ish string derived from the message
    // (not secure; this is only to allow dev usage over LAN without HTTPS)
    let hash = 0;
    for (let i = 0; i < message.length; i++) hash = (hash << 5) - hash + message.charCodeAt(i) | 0;
    return btoa(String(Math.abs(hash))).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
  }
...
async function createJwt(payload = {}, expiresInSeconds = 3600) {
  ...
  const signature = await signHMAC(signingInput, JWT_SECRET);
