...
function getMateriPreviewType(fileName, mimeType) {
  ...
  const [materiInputType, setMateriInputType] = useState("file"); // "file" | "link"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (materiInputType === "file" && formData.materi instanceof File) {
        filesToUpload.push({ name: "Materi", file: formData.materi });
      }
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                ...
                <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200 self-start sm:self-auto">
                  <button
                    type="button"
                    onClick={() => setMateriInputType("file")}
                  ...
                  <button
                    type="button"
                    onClick={() => setMateriInputType("link")}
                    className={`px-3 py-1 text-xs font-medium rounded-md transition-all flex items-center gap-1.5 ${
